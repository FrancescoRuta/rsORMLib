use mysql_async::prelude::{FromValue, ConvIr, ToValue};
...
impl FromValue for DBTime {
	...
	fn from_value_opt(v: mysql_async::Value) -> Result<Self, mysql_async::FromValueError> {
		Ok(
			if let mysql_async::Value::Time(is_negative, d, h, m, s, _) = v {
...
impl ConvIr<DBTime> for DBTime {
	fn new(v: mysql_async::Value) -> Result<Self, mysql_async::FromValueError> {
		if let mysql_async::Value::Time(is_negative, _, hours, minutes, seconds, _) = v {
			let hours = if is_negative { -(hours as i16) } else { hours as i16 };
			Ok(DBTime(hours, minutes, seconds))
		} else {
			Err(mysql_async::FromValueError(v))
		}
	}
...
impl FromValue for DBDate {
	...
	fn from_value_opt(v: mysql_async::Value) -> Result<Self, mysql_async::FromValueError> {
		Ok(
			if let mysql_async::Value::Date(y, m, d, _, _, _, _) = v {
				DBDate(y as i32, m, d)
			} else {
...
impl ConvIr<DBDate> for DBDate {
	fn new(v: mysql_async::Value) -> Result<Self, mysql_async::FromValueError> {
		if let mysql_async::Value::Date(year, month, day, _, _, _, _) = v {
...
impl FromValue for DBDateTime {
	...
	fn from_value_opt(v: mysql_async::Value) -> Result<Self, mysql_async::FromValueError> {
		Ok(
			if let mysql_async::Value::Date(y, m, d, h, min, s, _) = v {
...
impl ConvIr<DBDateTime> for DBDateTime {
	fn new(v: mysql_async::Value) -> Result<Self, mysql_async::FromValueError> {
		if let mysql_async::Value::Date(year, month, day, hour, minutes, seconds, _) = v {
